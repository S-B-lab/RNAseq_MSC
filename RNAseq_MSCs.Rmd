...
dimension_plot_function <- function(data_set, dim_reduction_type, textboolean=TRUE) {
  dim_plot <- ggplot(data_set, aes(V1, V2, color=group, shape=group))+
    ...
    annotate(geom="text", x=0, y=0, label=dim_reduction_type, size=7, fontface=2, alpha=0.7)
  
    if(textboolean==T){dim_plot <- dim_plot + geom_text(aes(label=ID), nudge_x = 7, show.legend = F)}
...
dimension_plot_function(sammon_mapping_plot_data, "Sammon's Non-\nLinear Mapping")
...
dimension_plot_function(MDS_classical_fit, "Classical MDS")
...
dimension_plot_function(MDS_nonmetric_fit, "Nonmetric MDS")
...
dimension_plot_function(plda, "Linear Discriminant Analysis", textboolean = F)

#E. Flexible discriminant analysis
library(mda)
fda_data <- data.table(t(dim_data), group=DT_meta_data[, group])
fda_fit <- fda(group~., data=fda_data)
fda_plot_data  <- data.table(fda_fit$means[1:3,1:2], rownames(fda_fit$means))
names(fda_plot_data) <- c("V1", "V2", "group")

dimension_plot_function(fda_plot_data, "FDA", textboolean = F)
...
dimension_plot_function(mda_plot_data, "MDA", textboolean = F)

# G. Singular value decomposition
svd_data <- data.table(t(dim_data), group=DT_meta_data[, factor(group)])
svd_fit <- svd(svd_data[,1:1000])
svd_plot_data <- data.table(V1=svd_fit$u[, 1], V2=svd_fit$u[, 2], group=DT_meta_data[, factor(group)])

dimension_plot_function(svd_plot_data, "SVD", textboolean = F)
...
dimension_plot_function(MLFA_plot_data, "MLFA", textboolean = F)

#Kernel PCA
library(kernlab)
kpca_fit <- kpca(t(dim_data))
...
dimension_plot_function(kpca_plot_data, "kPCA", textboolean = F)
...
```{r echo=F, warning=F, message=F, error=F, fig.height=6, fig.width=10}
#Clustering
#-> kör på representativ dimensionality reduction (t ex t-SNE)
