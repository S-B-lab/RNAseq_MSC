...
#+ testa hypotes om lysosomen -> finns det gener som är relaterade till denna i heatmapen?
...
gene_annotations <- data.table(OrganismDbi::select(Mus.musculus, keys=DT_read_count[,as.character(ENSEMBL_ID)], columns="SYMBOL", keytype = "ENSEMBL"))
...
DT_principal_comp <- merge(DT_principal_comp, DT_meta_data[,c(1:3)], by="ID")
...
#1. Creating group_names to avoid repetition in plot
group_names <- c("MSC[in vitro]", "MSC[Naive]", "MSC[SCI]")

#2. Creating ellipse data
#PCA
...
PCA_plot <- ggplot(DT_principal_comp, aes(PC1, PC2))+
  ...
  annotate("text", x=-30, y=10, label="MSC[Naive]", size=6, fontface=2)+
...
tSNE_plot <- function(legend_status){
  ...
  geom_segment(aes(x=-70, xend=70, y=0, yend=0), size=0.2)+
  geom_segment(aes(x=0, xend=0, y=-80, yend=70), size=0.2)+
  ...
  annotate("text", x=50, y=35, label="MSC[Naive]", size=6, fontface=2)+
  annotate("text", x=10, y=70, label="MSC[SCI]", size=6, fontface=2)+
  annotate("text", x=-30, y=-50, label="MSC[in vitro]", size=6, fontface=2)+
  
  annotate(geom="text", x=0, y=0, label="tSNE", size=10, fontface=2, alpha=0.7)
  
  return(plot_out)
  
}

#3. Fancy 3D plot (for PCA & t-SNE)
...
scatter3D_fancy <- function(x, y, z,..., colvar = z, plot_color_palette=color_palette)
  {
   panelfirst <- function(pmat) {
      ...
      scatter2D(XY$x, XY$y, colvar = colvar, pch = ".", 
              cex = 3, add = TRUE, colkey = F, col = plot_color_palette)
...
grid.arrange(arrangeGrob(PCA_plot, tSNE_plot("none"), ncol=2), PCA_tSNE_legend, nrow=2, heights=c(5,1))

```

```{r echo=F, warning=F, message=F, error=F, fig.height=6, fig.width=10}

#3D PCA (biological replicates)
...
DT_tSNE_raw <- t(DT_lcpm[ENSEMBL_ID %in% gene_variance_top500[,ENSEMBL_ID],-(1:2)])
DT_tSNE <-  Rtsne(as.matrix(DT_tSNE_raw), check_duplicates=FALSE, pca=TRUE, perplexity=3, theta=0, dims=3)
tSNE_plot_data <- data.table(DT_tSNE$Y, group=DT_meta_data[, group], ID=DT_meta_data[, ID])

#3D t-SNE (biological replicates)
...
```{r echo=F, warning=F, message=F, error=F}
#1. PCA bootstrap function: boostrap from top 500 genes with highest variance and calculate PC1 and PC2 
...
#3D PCA bootstrapped (with 3D-coordinates)
...
#3D PCA bootstrapped (withou 3D-coordinates)
scatter3D_fancy(DT_pca_boot$PC1, DT_pca_boot$PC2, DT_pca_boot$PC3, 
            colvar=as.integer(DT_pca_boot$group), 
            pch=19, 
            col=color_palette, 
...
dim_data <- DT_lcpm[ENSEMBL_ID %in% gene_variance_top1000[,ENSEMBL_ID],-(1:2)]

#2.Plotting function for dimensionality reduction
dimension_plot_function <- function(data_set, dim_reduction_type, legend.position="none", textboolean=TRUE, cluster_type="none") {
  ...
  dim_plot <- ggplot(data_set, aes(V1, V2))+
    geom_segment(aes(x=data_set[, min(V1)]*1.2, xend=data_set[, max(V1)]*1.2, y=data_set[, mean(V2)], yend=data_set[, mean(V2)]), color="black", size=0.1, alpha=0.5)+
    geom_segment(aes(x=data_set[, mean(V2)], xend=data_set[, mean(V2)], y=data_set[, min(V2)]*1.2, yend=data_set[, max(V2)]*1.2), color="black", size=0.1, alpha=0.5)+
    ...
    annotate(geom="text", x=data_set[, mean(V1)], y=data_set[, mean(V2)], label=dim_reduction_type, size=7, fontface=2, alpha=0.7)
...
#Setting train structure for row selection
train <- sample(1:22, round(22*0.6))
#A. SAMMON'S NON-LINEAR MAPPING 
library(MASS)
sammon_mapping_fit <- sammon(dist(as.matrix(t(dim_data))))
...
#B. Classical MDS
MDS_classical_fit <- cmdscale(dist(t(dim_data)), eig=TRUE, k=2) 
MDS_classical_fit <- data.table(MDS_classical_fit$points)
MDS_classical_fit[,`:=`(ID=DT_meta_data[, ID], group=DT_meta_data[,group])]

#C. Non-metric MDS
MDS_nonmetric_fit <- isoMDS(dist(t(dim_data)), k=2) 
MDS_nonmetric_fit <- data.table(MDS_nonmetric_fit$points)
MDS_nonmetric_fit[,`:=`(ID=DT_meta_data[, ID], group=DT_meta_data[,group])]

# #D. Linear discriminant analysis
# da_data <- data.table(t(dim_data))
# da_data[, group:=DT_meta_data[, group]]
# #training model on 80 % of replicates
# train <- sample(1:22, round(22*0.6))
# da_train <- lda(formula=group~., data=da_data, subset=train)
# #predicting 
# plda <- predict(object=da_train, newdata = da_data[-train, ])
# plda <- data.table(group=plda$class, V1=plda$x[,1], V2=plda$x[,2])

#E. Flexible discriminant analysis
library(mda)
fda_data <- data.table(t(dim_data), group=DT_meta_data[, group])
fda_fit <- fda(group~., data=fda_data)
fda_plot_data  <- data.table(fda_fit$means[, 1], fda_fit$means[, 2], group=rownames(fda_fit$means))
...
dimension_plot_function(kpca_plot_data, "kernel\nPCA", textboolean = F, legend.position = "bottom")
grid.arrange(arrangeGrob(dimension_plot_function(kpca_plot_data, "kernel\nPCA", textboolean = T), dimension_plot_function(MDS_classical_fit, "Classical MDS"), ncol=2))

grid.arrange(arrangeGrob(dimension_plot_function(MDS_nonmetric_fit, "Nonmetric MDS"), dimension_plot_function(tSNE_plot_data, "t-SNE"), ncol=2))

grid.arrange(arrangeGrob(dimension_plot_function(mda_plot_data, "MDA", textboolean = F), dimension_plot_function(fda_plot_data, "FDA", textboolean = F), ncol=2))

#grid.arrange(arrangeGrob(dimension_plot_function(plda, "LDA", textboolean = F), dimension_plot_function(sammon_mapping_plot_data, "Sammon's Non-\nLinear Mapping"), ncol=2))

grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(svd_plot_data, "SVD", textboolean = T), dimension_plot_function(MLFA_plot_data, "MLFA", textboolean = T), ncol=2), dimension_legend, nrow=2, heights=c(10,1)))
...
knn_function <- function(data_set){
  knn_predict <- class::knn(data_set[train, !"group"], data_set[-train, !"group"], cl=data_set[train, "group"], k=3)  
  data_set[-train, class::knn:=knn_predict]
...
#8A. Hierarchical clustering: 1000 genes with highest variance

#1. Selecting the 1000 genes with highest variance 
gene_variance_top1000 <- gene_variance[order(-variance)][1:1000]
#2. Creating a dissimilarity structure for the 1000 genes with highest variance
setkey(DT_lcpm, ENSEMBL_ID)
DT_dist <- dist(t(DT_lcpm[ENSEMBL_ID %in% gene_variance_top1000[,ENSEMBL_ID],-(1:2)]))
#3. Hierarchical clustering on the dissimilarity structure
DT_hclust <- hclust(DT_dist)

#8B. Circular packing (hiding the first level of hierarchy) representation
#Installation guide:
...
#3. Create and arrange for group and ID addition to the data-frame
...
#4. Adjusting data frame prior to plotting 
...
hierarchical_clustering_circular_1000
...
#Expectation Maximum
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Expectation Maximum\n[PCA]", cluster_type = "EM"), dimension_plot_function(tSNE_cluster_data, "Expectation Maximum\n[tSNE]", cluster_type = "EM"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
#Spectral Clustering
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Spectral Clustering (3Cs)\n[PCA]", cluster_type = "SC"), dimension_plot_function(tSNE_cluster_data, "Spectral Clustering(3Cs)\n[tSNE]", cluster_type = "SC"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
#K-Means Clustering
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "K-Means Clustering (3Cs)\n[PCA]", cluster_type = "KM"), dimension_plot_function(tSNE_cluster_data, "K-Means Clustering (3Cs)\n[tSNE]", cluster_type = "KM"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
#K-Nearest Neighbour
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "K-Nearest Neighbour (train:test-60:40)\n[PCA]", cluster_type = "KNN"), dimension_plot_function(tSNE_cluster_data, "K-Nearest Neighbour (train:test-60:40)\n[tSNE]", cluster_type = "KNN"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
#Support Vector Machine
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Support Vector Machine (train:test-60:40)\n[PCA]", cluster_type = "SVM"), dimension_plot_function(tSNE_cluster_data, "Support Vector Machine (train:test-60:40)\n[tSNE]", cluster_type = "SVM"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
#Affinity Propagation
grid.arrange(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Affinity Propagation\n[PCA]", cluster_type = "AP"), dimension_plot_function(tSNE_cluster_data, "Affinity Propagation\n[tSNE]", cluster_type = "AP"), ncol=2), cluster_legend, nrow=2, heights=c(6,1))
...
#Sorting each principal component separately
...
#Converting PC to a factor variable
...
#Adjusting values for easier plotting
...
#6. Creating dataset for circular packing representation
...
hierarchical_clustering_circular_Fstatistica <- ggraph(DT_toptableF_circularpacking, layout='circlepack')+
  ...
  theme_void()+
  theme(legend.position="FALSE")+
  geom_node_text(aes(label=ID, filter=leaf), size=2.5, fontface=2)
...
hierarchical_clustering_circular_Fstatistica
```
...
entrez_ID <- data.table(OrganismDbi::select(Mus.musculus, keys=DT_read_count_filtered[,as.character(ENSEMBL_ID)], columns="ENTREZID", keytype = "ENSEMBL"))
...
```{r echo=F, warning=F, message=F, error=F, fig.width=30, fig.height=30}
#Define the genes which are in the three clusters
  # rank the genes on LogFC and/or p-value -> present
#Make circular plot combining clusters and samples
#Network representation?

#1. Retrieving data from
heatmap_data <- invisible(heatmap_plot_function(DT_logFC_significant_lcpm[, 3:length(DT_logFC_significant_lcpm)], 2, DT_meta_data[,group], "both", DT_logFC_significant_lcpm[, as.character(ifelse(is.na(SYMBOL), ENSEMBL_ID, SYMBOL))]))
...
#3. Changing row order to match layout of heatmap
...
#4. Identifying the significant cluster
...
#Gör clusteranalys -> ranka på logFC & p-value
DT_logFC_significant_lcpm_heatmap[cluster==2]
