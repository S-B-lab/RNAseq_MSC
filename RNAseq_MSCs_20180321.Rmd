---
title: "Transcriptomal Changes in MSCs Following Exposure to Spinal Cord Injury"
author: "Ramil Hakim"
...
* **Reduction of dimension and clustering reveals good separation between the three study groups**

* **MSCs exposed to spinal cord injury is similar but distinctly different from MSCs exposed to naive spinal cord**

* **~10000 genes are differentially expressed between MSC[SCI] and MSC[Naive]**

* **MSC[Naive] are similar to MSC[in vitro] while MSC[SCI] are different from MSC[in vitro]**
...
* **Heatmap analysis reveals 7 clusters separating MSC[in vitro] from the other two study groups. 700 genes in cluster 2 explain most of the difference between MSC[SCI] and MSC[naive]**
...
###SECTION OUTPUT
grid.arrange(arrangeGrob(density_raw_plot, left=textGrob("A", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(density_filtered_plot, left=textGrob("B", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), ncol=2, widths=c(3,3), bottom=textGrob("log-CPM", gp=gpar(fontsize=17, fontface="bold")), left=textGrob("Density", gp=gpar(fontsize=17, fontface="bold"), rot=90))
...
###SECTION OUTPUT
grid.arrange(arrangeGrob(log_cpm_distribution_function(DT_lcpm_notNormalised_melt, "RAW"), left=textGrob("A", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(log_cpm_distribution_function(DT_lcpm_melt, "NORMALIZED"), left=textGrob("B", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), ncol=2, bottom=textGrob("Sample", gp=gpar(fontsize=17, fontface="bold")), left=textGrob("log-CPM", gp=gpar(fontsize=17, fontface="bold"), rot=90))
...
* **Principal Component Analysis (PCA):** Feature extraction with linear approach. Variation in the low-dimensional representation is maximized. 1) Variance-covariance matrix is calculated, 2) eigenvalues (principal components) and eigenvectors of the covariance matrix are determined. The eigenvectors belonging to the largest eigenvalues can be used to reconstruct a large fraction of the variance of the original data.     

* **t-distributed Stochastic Neighbour Embedding (tSNE):** A non-linear approach. Keeps dissimilar points apart and similar close in comparison to PCA which preserves dissimilarities. Algorithm has two main steps: 1) probability distribution is constructed on the high-dimensional data such that similar points are likely to be picked and vice versa. 2) Algorithm then defines a similar distribution but in lower-dimensional space and 3) the divergence between the two distributions is minimized. Perplexity is comparable to the number of nearest neighbours k in other techniques. Theta is a speed accrucacy trade-off.

* **Sammon's non-linear mapping:** A non-linear approach. Maps high-dimension to low-dimension by preserving the structure of inter-point distances in high dimensions even in low dimensions. Minimizes a stress function (Sammaons error/stress).
...
* **Singular Value Decomposition (SVD):** The matrix A(mxn) is factorized into a unitary matrix U (mxm), a positive diagnonal matrix (mxn) and a unitary matrix V(nxn). If the conjugate transpose of a square matrix is also its inverse the matrix is said to be unitary. 

* **Kernel PCA:** Extension of PCA by using kernel methods (use kernel methods to compute the inner products between all pairs of data in the feature space). The data points are first mapped onto a non-linear feature space, then the PCA is conducted as usual. 
...
###SECTION OUTPUT
grid.arrange(arrangeGrob(PCA_plot_prop.var_function(DT_principal_comp_summary,"var.prop", 1), left=textGrob("A", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(PCA_plot_prop.var_function(DT_principal_comp_summary,"var.prop.cumsum", 11), left=textGrob("B", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=15))), ncol=2, bottom=textGrob("Principal component", gp=gpar(fontsize=17, fontface="bold")), left=textGrob("Proportion variance explained", gp=gpar(fontface="bold", fontsize=17), rot=90))
...
**Fig 5.** _Figure reports the samples in low dimensional space following dimensionality reduction with PCA and tSNE for the 500 genes with the highest variance. Ellipses are added to the samples for easier study group recognition._
...
**Fig 6.** _Figure reports the samples in three-dimensional space following dimensionality with PCA (Fig 6A) and t-SNE (Fig 6B). The third dimension is added in order to visualize the separation when the second and third component/variable is used. Two-dimensional representations are added on the walls in addition to the three-dimensional representation._
...
**Fig 7.** _Figure reports the three-dimensional representation of 1000 bootstrap replicates of PCA. Fig 7A reports with three-dimensional coordinates while Fig 7B does not._
...
#E. Maximum likelihood factor analysis
...
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(MDS_classical_fit, "Multidimensional Scaling\n[classical]"), left=textGrob("A", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(MDS_nonmetric_fit, "Multidimensional scaling\n[nonmetric]"), left=textGrob("B", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), dimension_legend, nrow=2, heights=c(5,1))

grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(sammon_mapping_plot_data, "Sammon's Non-\nLinear Mapping"), left=textGrob("C", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(kpca_plot_data, "kernel\nPCA", textboolean = T), left=textGrob("D", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), dimension_legend, nrow=2, heights=c(5,1))

grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(svd_plot_data, "Singular Value\nDecomposition", textboolean = T), left=textGrob("E", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(MLFA_plot_data, "Maximum Likelihood\nFactor Analysis", textboolean = T), left=textGrob("F", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), dimension_legend, nrow=2, heights=c(5,1))

```

**Fig 8.** _Figure panel reports common dimensionality reduction algorithms applied on the dataset._

***

#Clustering Algorithms

* **Expectation maximisation:** 1) Clusters are assigned randomly onto the data. Clusters are given mean, covariance (allowing elliptical shape) and weight (size, i.e. number of observations). 2) A probability for an observation to belong to a cluster is calculated for each observation. The probability for an observation to belong to a cluster is given by a multivariate Gaussian probability distribution. 3) Mean, covariance and size is re-estimated for each cluster using the probabilties for each observation as weight. Steps are repeated until convergence. Algorithm might have to be run several times since the initial allocation of clusters is random.    

* **Spectral clustering:** Complicated. 
...
* **Affinity propagation:** Requires two inputs: 1) similarites between data points, 2) preferences representing each data points suitabiity to be an exemplar (i.e. cluster centroid). The algorithm then refines a randomly-chosen initial set of exemplars by maximizing the total similarity between data points and their exemplars.
...
* **Support vector machine:** A linear classifier using a hyperplane. 1) The hyperplane separating the clusters is identified, 2) the margin is maximized (the plane is as far away from both clusters as possible). The hyperplane which best separates the clusters is called the maximum-margin hyperplane. Outliers (observations making separation with hyperplane impossible) are igonerd by the SVM. If a linear separation still is not possible the SVM algorithm transfers the low-dimensional data to higher dimensional data using kernel methods. 

* **Agglomerative Hierarchical Clustering:** Clusters on distance starting with the two observations being closest. 
...
arrangeGrob(grob(myplclust(DT_hclust, labels=paste(DT_meta_data[,group]," ","(", DT_meta_data[,ID],")", sep=""), main="", ylab="", lab.col=brewer.pal(11, "RdBu")[c(1,2,10)][as.numeric(DT_meta_data[,group])], font=2, cex=1, hang=0.05)), left=textGrob("A", vjust=-15, hjust=0, gp=gpar(fontface=2, fontsize=17))),

arrangeGrob(hierarchical_clustering_circular_1000, left=textGrob("B", vjust=-15, hjust=0, gp=gpar(fontface=2, fontsize=17))), ncol=2))

```

**Fig 9.** _Figure reports hierarchical clustering based on te 500 most variable genes. Fig 9A reports the clustering using a dendrogram while Fig 9B reports the same clustering using a circular packing plot._
...
#left=textGrob("F", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))
#K-Nearest Neighbour
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "K-Nearest Neighbour\n(train:test-60:40)\n[PCA]", cluster_type = "KNN"), left=textGrob("A", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "K-Nearest Neighbour\n(train:test-60:40)\n[tSNE]", cluster_type = "KNN"), left=textGrob("B", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#Support Vector Machine
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Support Vector Machine\n(train:test-60:40)\n[PCA]", cluster_type = "SVM"), left=textGrob("C", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "Support Vector Machine \n(train:test-60:40)\n[tSNE]", cluster_type = "SVM"), left=textGrob("D", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#K-Means Clustering
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "K-Means Clustering (3Cs)\n[PCA]", cluster_type = "KM"), left=textGrob("E", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "K-Means Clustering (3Cs)\n[tSNE]", cluster_type = "KM"), left=textGrob("F", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#Expectation Maximum
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Expectation Maximum\n[PCA]", cluster_type = "EM"), left=textGrob("G", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "Expectation Maximum\n[tSNE]", cluster_type = "EM"), left=textGrob("H", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#Spectral Clustering
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Spectral Clustering (3Cs)\n[PCA]", cluster_type = "SC"), left=textGrob("I", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "Spectral Clustering(3Cs)\n[tSNE]", cluster_type = "SC"), left=textGrob("J", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#Affinity Propagation
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Affinity Propagation\n[PCA]", cluster_type = "AP"), left=textGrob("K", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "Affinity Propagation\n[tSNE]", cluster_type = "AP"), left=textGrob("L", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
...
**Fig 12.** _Figure reports the top ten loadings (genes) using absolute values for the first four principal components. No label indicates NA value (no symbol available). _
...
**Fig 15.** _Figure reports the mean-variance relationship pre -and post application of the voom function. Fig 9A reports the average log-CPM against the quarter root of the variance. Fig 9B reports average log-CPM against the log2(st.dev). Blue line reports the average log2(st.dev). The red line is a linear trend fitted to the black dots. Each black dot represents a gene. Fig 9A illustrates that the variance is decresing when the average expression is increasing. In Fig 9B the dependency is removed and the mean variance is unchanged when the average expression increases._      
...
arrangeGrob(grob(myplclust(DT_toptableF, labels = paste(DT_meta_data[,group]," ","(", DT_meta_data[,ID], ")", sep=""), lab.col = brewer.pal(11,"RdBu")[c(1,2,10,11)][as.numeric(DT_meta_data[,group])], main=" ", ylab=" ", font=2, cex=1, hang=0.05)), left=textGrob("A", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15))),

arrangeGrob(left=textGrob("B", vjust=-12, hjust=-3, gp=gpar(fontface=2, fontsize=15)),hierarchical_clustering_circular_Fstatistica), ncol=2))
...
**Fig 17. Number of differentially expressed genes for each contrast**
...
**Fig 17.** _Figure reports a venn diagram containing the number of differentially expressed genes for each contrast._
...
**Fig 18. Mean difference -and volcano plot**
...
mean_difference_plot <- ggplot(DT_mean_difference, aes(AveExpr,logFC))+
  ...
  annotate("text", x=12, y=5.5, label=nrow(DT_mean_difference[logFC>=1 & adj.P.Val>1e-6]), color="dodgerblue4", fontface=2, alpha=0.8, size=4.5)+
    annotate("text", x=12, y=-5.5, label=(nrow(DT_mean_difference[logFC<=-1 & adj.P.Val>1e-6])+2), color="dodgerblue4", fontface=2, alpha=0.8, size=4.5)+

  annotate("text", x=12, y=-3.5, label=(nrow(DT_mean_difference[logFC<=-1 & adj.P.Val<1e-6])+2), color="red", fontface=2, alpha=0.8, size=4.5)+
  annotate("text", x=12, y=3.5, label=nrow(DT_mean_difference[logFC>=1 & adj.P.Val<1e-6]), color="red", fontface=2, alpha=0.8, size=4.5)+
...
###SECTION OUTPUT
plot_grid(arrangeGrob(mean_difference_plot, left=textGrob("A", vjust=-15, hjust=-2, gp=gpar(fontsize=17, fontface=2))), arrangeGrob(volcano_plot, left=textGrob("B", vjust=-15, hjust=-2, gp=gpar(fontsize=17, fontface=2))))
...
**Table 5. GO terms and KEGG pathways using FDR<1e-6**
...
```{r echo=F, warning=F, message=F, error=F, fig.width=30, fig.height=35}
...
**Fig 19.** _Figure reports a heatmap with hierarchical clustering (indicated with dendrograms) using log-CPM values. Only significantly differentially expressed genes are included (and genes with NA symbols were removed)._ 
...
###SECTION OUTPUT
kable(data.table(cluster2[order(logFC)][1:10][,`:=`(logFC=round(logFC, 2), adj.P.Val=format(adj.P.Val, scientific=T, digits=2))], cluster2[order(-logFC)][1:10][,`:=`(logFC=round(logFC, 2), adj.P.Val=format(adj.P.Val, scientific=T, digits=2))]), align="c", col.names = rep(c("Gene", "log2(fold change)", "P-value (adjusted)"), 2))
...
circos.par(gap.after = c(rep(5, nrow(chord_data)-1), 15, rep(5, ncol(chord_data)-1), 15))
...
chordDiagram(chord_data, 
             ...
             link.rank = rank(chord_data),
             annotationTrack = c("name","grid"))
circos.clear()

```

**Fig 20.** _Figure reports a chord diagram which relates the study groups with the associated clusters._
...
**Fig 21.** _Figure reports hierarchical clustering using multiscale boostrap resampling with 1000 replicates. Numbers with red color are the approximately unbiased (AU) p-values while blue numbers represent bootstrap probability (BP). AU >95 and BP >70 are considered to be significant clusters. Red boxes indicate clusters with AU >95._
