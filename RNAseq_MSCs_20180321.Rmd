...
grid.arrange(arrangeGrob(dimension_plot_function(kpca_plot_data, "kernel\nPCA", textboolean = T), dimension_plot_function(MDS_classical_fit, "Multidimensioanl Scaling\[classical]"), ncol=2), dimension_legend, nrow=2, heights=c(5,1))

grid.arrange(arrangeGrob(dimension_plot_function(MDS_nonmetric_fit, "Multidimensional scaling\n[nonmetric]"), dimension_plot_function(sammon_mapping_plot_data, "Sammon's Non-\nLinear Mapping"), ncol=2), dimension_legend, nrow=2, heights=c(5,1))

grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(svd_plot_data, "Singular Value Decomposition", textboolean = T), dimension_plot_function(MLFA_plot_data, "Maximum Likelihood\nFactor Analysis", textboolean = T), ncol=2), dimension_legend, nrow=2, heights=c(5,1)))
...
hierarchical_clustering_circular_500 <- ggraph(DT_hclust_circularpacking, layout='circlepack')+
...
```{r echo=F, warning=F, message=F, error=F, fig.height=10, fig.width=14}
...
invisible(heatmap_data <- heatmap_plot_function(DT_logFC_significant_lcpm[, 3:length(DT_logFC_significant_lcpm)], 2, DT_meta_data[,group], "both", DT_logFC_significant_lcpm[, as.character(ifelse(is.na(SYMBOL), ENSEMBL_ID, SYMBOL))]))
...
invisible(h <- heatmap(as.matrix(DT_logFC_significant_lcpm[,3:length(DT_logFC_significant_lcpm)]), keep.dendro=TRUE))
