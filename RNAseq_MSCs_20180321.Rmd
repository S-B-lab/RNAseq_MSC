...
scatter3D_fancy <- function(x, y, z,..., colvar = z, plot_color_palette=color_palette)
  {
   panelfirst <- function(pmat) {
      ...
      scatter2D(XY$x, XY$y, colvar = colvar, pch = ".", 
              cex = 6, add = TRUE, colkey = F, col = plot_color_palette, alpha=0.8)
...
```{r echo=F, warning=F, message=F, error=F, fig.height=6, fig.width=10}
#1. PCA bootstrap function: boostrap from top 500 genes with highest variance and calculate PC1, PC2 & PC3 
...
invisible(capture.output(sammon_mapping_fit <- sammon(dist(as.matrix(t(dim_data))))))
...
#C. Non-metric MDS
MDS_nonmetric_fit <- invisible(isoMDS(dist(t(dim_data)), k=2)) 
...
#4. K-Nearest Neighbour (2 neighbours)
...
#Creating a phylo-object from a hclust-object
...
test <- merge(DT_logFC_significant_lcpm_heatmap[cluster==2, "SYMBOL"], DT_mean_difference[, .(ID, logFC, adj.P.Val)], by.x="SYMBOL", by.y="ID")
test <- test[order(adj.P.Val)]
...
test <- data.frame(group=c(rep("SCI", 3), rep("naive", 3), rep("invitro", 4)), cluster=c(rep(c(2,4,6), 2), c(1,3,5,7)))
names(test) <- c("group", "cluster")
test <- matrix(with(test, table(group, cluster)), ncol=7)
colnames(test) <- paste(rep("C", 7), 1:7, sep=":") 
rownames(test) <- c("In vitro", "Naive", "SCI")

dev.off()
grid_col <- c(`In vitro` = color_palette[1], Naive = color_palette[3], SCI = color_palette[2],
    `C:1` = chord_palette_cluster[10], `C:2` = chord_palette_cluster[8], `C:3` = chord_palette_cluster[10], `C:4` = chord_palette_cluster[8], `C:5` = chord_palette_cluster[10], `C:6` = chord_palette_cluster[8], `C:7`=chord_palette_cluster[10])
