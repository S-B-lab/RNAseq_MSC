...
scatter3D_fancy <- function(x, y, z,..., colvar = z)
  {
   panelfirst <- function(pmat) {
      ...
      scatter2D(XY$x, XY$y, colvar = colvar, pch = ".", 
              cex = 3, add = TRUE, colkey = F, col = color_palette)
...
knn_function <- function(data_set){
  knn_predict <- knn(data_set[train, !"group"], data_set[-train, !"group"], cl=data_set[train, group], k=3)  
...
PCA_cluster_data <- merge(PCA_cluster_data, AP_PCA, by="ID")

tSNE_cluster_data <- cbind(tSNE_plot_data, EM=EM_PCA, SC=SC_PCA, KM=k_means_tSNE, KNN=KNN_tSNE, SVM=SVM_tSNE)
tSNE_cluster_data <- merge(tSNE_cluster_data, AP_PCA, by="ID")
...
#3D illustration of clustering (SC, AP, K-means) on bootstrapped PCA-data
#Clustering bootstrapped PCA data - 100 random samples for each ID
DT_pca_boot_sample <- do.call(rbind, lapply(split(DT_pca_boot[,c(1:4)], DT_pca_boot[,"ID"]), function(subset){subset[sample(.N, 100)]}))

#A.Spectral Clustering
#SC_PCA_boot <- specc(as.matrix(DT_pca_boot_sample[,c(2:4)]), centers=3)
#B. Affinity propagation

#color=cluster
#add ID as text
scatter3D_fancy(PCA_cluster_data$V1, PCA_cluster_data$V2, PCA_cluster_data$V3, colvar = as.integer(PCA_cluster_data$group), 
            pch=19, 
            col = color_palette, 
            alpha=0.7, 
            xlab="PC1",
