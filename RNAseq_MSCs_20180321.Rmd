...
dimension_plot_function <- function(data_set, dim_reduction_type, legend.position="none", textboolean=TRUE) {
  dim_plot <- ggplot(data_set, aes(V1, V2, color=group, shape=group))+
    ...
    if(textboolean==T){dim_plot <- dim_plot + geom_text(aes(label=ID), show.legend = F)}
...
knn_function <- function(data_set){
  ...
  out <- data.table(data_set[-train, !"group"], knn_predict)
  return(out)
}
...
library(e1071)
svm_function <- function(dataset){
  svm_fit <- svm(group~PC1+PC2+PC3, data=dataset[train])
  svm_pred <- predict(svm_fit, dataset[-train])
  return(svm_pred)
}

#Creating cluster datasets
PCA_cluster_data <- cbind(DT_principal_comp[, c(1:4, 24)], EM=EM_PCA, SC=SC_PCA, KM=k_means_PCA)
PCA_cluster_data <- merge(PCA_cluster_data, AP_PCA, by="ID")

tSNE_cluster_data <- cbind(tSNE_plot_data, EM=EM_PCA, SC=SC_PCA, KM=k_means_PCA)
tSNE_cluster_data <- merge(tSNE_cluster_data, AP_PCA, by="ID")

#8A. Hierarchical clustering: 1000 genes with highest variance
#draw with dendrogram and with circular packing (1 setup for 1000 most variable genes och 1 setup for highest F-value)
...
#8B. Circular packing (hiding the first level of hierarchy)
...
#                    configure.args=c('--with-udunits2-lib=/usr/lib/R/library'))
